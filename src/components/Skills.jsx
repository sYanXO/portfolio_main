                    ...
                    {skills.map((skillGroup, idx) => (
                        <Reveal key={idx} delay={idx * 100}>
                            <div className={`p-6 md:p-8 rounded-2xl h-full transition-all duration-300 hover:bg-opacity-80 ${theme.cardGlass}`}>
                                <h3 className={`text-lg md:text-xl font-bold mb-5 md:mb-6 border-b border-[#333333] pb-4`}>{skillGroup.category}</h3>
                                <div className="flex flex-wrap gap-3">
                                    {skillGroup.items.map((skill) => (
                                        <span
                                            key={skill}
                                            className={`px-3.5 py-2 rounded-lg text-sm font-bold transition-all hover:scale-105 bg-[#333333] text-[#888888] hover:bg-[#EDEDED] hover:text-[#000000]`}
                                        >
                                            {skill}
                                        </span>
                                    ))}
